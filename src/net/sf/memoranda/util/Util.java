	...
	public static void runBrowser(String url) {
		...
		try {
			if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Action.BROWSE)) {
				try {
					// will open the default browser on "Supported
					// Desktops"(windows and ubuntu probably)
				...
				if (os.indexOf("mac") >= 0) {
					// will open the default browser on most if not all macs
					runtime.exec("open " + url);
				} else {
					// will open the default browser on X-based linux desktops
			...
			try {
				...
				new ExceptionDialog(e,
						"Failed to run an external web-browser application with command:<br><code>" + commandLine
								+ "</code>",
